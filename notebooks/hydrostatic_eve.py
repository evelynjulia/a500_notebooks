...
def calcScaleHeight(T,p,z):
    ...
    """
    dz=np.diff(z)
    TLayer=(T[1:] + T[0:-1])/2. # finds the average of two consecutive layers
...
for sounding_name,df in sound_dict.items():
    # grab the dataframe and get the sounding columns
    z=df['z'].values
    Temp=df['temp'].values
    press=df['press'].values
    ...
    Hbar= calcScaleHeight(TempL,pressL,zL)
    Hrho= calcDensHeight(TempL,pressL,zL)
    print(f"Pressure scale height for the {sounding_name} sounding is {Hbar*1.e-3:5.2f} km")
    print(f"Density scale height for the {sounding_name} is {Hrho*1.e-3:5.2f} km\n")
...
def calcPreciptWater(z, den, rmix, temp, press):
    """
    Calculate the the total precipitable water PW
    ...
    z = z
    den = den
    rmix = rmix # with units of kg/m^3
    temp = temp
    press = press
    Rd=287.058
...
#     rho_d = den # the dry air density
#     rho_t=(press)/(Rd*temp) # calculate total density
#     rho_v = rho_t - rho_d # the vapour density
    
#     # sum over 0 to ztop | divide by 10 to get value from mm to cm
#     PW = np.sum((rho_v[:-1])*dz)/10
...
# #### Question 3
# Use your function to print out W for all five soundings
...
for sounding_name,df in sound_dict.items():
    ...
    #z, den, rmix, temp, press
    # limit calculation to bottom 10 km
    hit=z<10000.
    zL,DensL, rmixL,tempL,pressL=(z[hit],Dens[hit],rmix[hit],temp[hit],press[hit])
    
    
    W= calcPreciptWater(zL, DensL, rmixL,tempL,pressL)
    print(f"Total precipitable water in the {sounding_name} sounding: {W:.4f} cm")
